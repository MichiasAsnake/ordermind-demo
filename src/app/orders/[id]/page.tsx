...
export default function OrderPage() {
    ...
    if (loading) return (
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
            <div className="text-gray-400 text-[14px] animate-pulse">Loading order…</div>
        </div>
    );

    if (!data || (data as { error?: string }).error) return (
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
            <div className="text-gray-500 text-[14px]">Order not found. <Link href="/" className="underline text-gray-700">Back to dashboard</Link></div>
        </div>
    );
            ...
            <header className="border-b border-gray-200 bg-white sticky top-0 z-10">
                <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <button onClick={() => router.back()} className="text-gray-400 hover:text-gray-600 transition-colors text-[14px]">← Back</button>
                        <span className="text-gray-200">|</span>
                        <div className="flex items-center gap-2">
                            <Link href="/" className="flex items-center gap-2">
                                <div className="w-7 h-7 rounded-lg bg-gray-900 flex items-center justify-center text-xs font-bold text-white">OM</div>
                                <span className="font-bold text-gray-900 text-[14px]">OrderMind</span>
                            </Link>
                        </div>
                    </div>
                    <a href="#" className="text-[13px] text-gray-400 hover:text-gray-600 border border-gray-200 rounded-lg px-3 py-1.5 transition-colors">
                        Open in ERP ↗
                    </a>
                </div>
            </header>
            ...
            <main className="max-w-5xl mx-auto px-6 py-8 space-y-6">
                    ...
                    <div className="flex items-start justify-between flex-wrap gap-4">
                        <div>
                            <div className="flex items-center gap-3 mb-2">
                                <h1 className="text-[22px] font-bold text-gray-900 font-mono">{order.jobNumber}</h1>
                            ...
                            <p className="text-[17px] font-semibold text-gray-800">{customer.company}</p>
                            <p className="text-[13px] text-gray-400 mt-0.5">{customer.contactPerson}</p>
                        </div>
                        <div className="grid grid-cols-3 gap-6 text-right">
                            <div>
                                <p className="text-[11px] text-gray-400 font-semibold uppercase tracking-widest mb-1">Value</p>
                                <p className="text-[20px] font-bold text-gray-900">{fmt$$(pricing.totalDue)}</p>
                            </div>
                            <div>
                                <p className="text-[11px] text-gray-400 font-semibold uppercase tracking-widest mb-1">Ship Date</p>
                                <p className={`text-[17px] font-bold ${isOverdue ? "text-red-600" : "text-gray-900"}`}>{order.requestedShipDate}</p>
                                {isOverdue && <p className="text-[11px] text-red-500 font-semibold">{Math.abs(order.daysUntilShip)}d overdue</p>}
                            </div>
                            <div>
                                <p className="text-[11px] text-gray-400 font-semibold uppercase tracking-widest mb-1">Status</p>
                                <p className="text-[14px] font-semibold text-gray-700">{order.status}</p>
                            </div>
                        </div>
                    </div>
                    {order.comment && (
                        <p className="mt-4 text-[13px] text-gray-500 bg-gray-50 rounded-lg px-4 py-3 border border-gray-100 italic">"{order.comment}"</p>
                    )}
                </section>
                ...
                {!risk.isHealthy && (
                    <section className="bg-white rounded-2xl border border-orange-100 shadow-sm overflow-hidden">
                        ...
                        <div className="px-6 py-5 grid md:grid-cols-2 gap-6">
                            <div>
                                <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-widest mb-3">What we found</p>
                            ...
                            <div className="md:border-l md:border-gray-100 md:pl-6">
                                <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-widest mb-3">Recommended actions</p>
                                <ul className="space-y-2">
                                    {aiSummary.actions.map((a, i) => (
                                        <li key={i} className="flex items-start gap-2 text-[13px] text-gray-800 font-medium">
                ...
                <div className="grid md:grid-cols-[1fr_320px] gap-6">
                    <div className="space-y-6">
                        ...
                        {/* ── Timeline ── */}
                        <section className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                            <h2 className="text-[14px] font-bold text-gray-900 mb-5">Order Timeline</h2>
                    ...
                    <div className="space-y-4">
                        ...
                        <section className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                            <h2 className="text-[13px] font-bold text-gray-900 mb-3 uppercase tracking-widest">Risk Factors</h2>
                            <div className="space-y-0">
                                ...
                                <div className="flex items-center justify-between py-2 border-b border-gray-100">
                                    <span className="text-[13px] text-gray-600">Days in Production</span>
                                    <span className={`text-[12px] font-semibold ${production.daysInProduction > 7 ? "text-red-500" : "text-gray-700"}`}>
                                        {production.daysInProduction}d {production.daysInProduction > 7 ? "⚠ Stalled" : ""}
                                    </span>
                                </div>
                                <div className="flex items-center justify-between py-2 border-b border-gray-100">
                                    <span className="text-[13px] text-gray-600">Complexity</span>
                                    <span className="text-[12px] font-semibold text-gray-700 capitalize">{production.complexity ?? "—"}</span>
                                </div>
                                <div className="flex items-center justify-between py-2">
                                    <span className="text-[13px] text-gray-600">Customer Priority</span>
                                    <span className={`text-[12px] font-bold ${customer.priority === "VIP" ? "text-amber-600" : "text-gray-700"}`}>
                                        {customer.priority}
                                    </span>
                                </div>
                            </div>
                        ...
                        {/* Quick facts */}
                        <section className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                            <h2 className="text-[13px] font-bold text-gray-900 mb-3 uppercase tracking-widest">Order Info</h2>
                            <div className="space-y-2 text-[13px]">
                                <div className="flex justify-between"><span className="text-gray-400">Entered</span><span className="text-gray-700">{order.dateEntered}</span></div>
                                <div className="flex justify-between"><span className="text-gray-400">Order #</span><span className="font-mono text-gray-700">{order.orderNumber}</span></div>
                                <div className="flex justify-between"><span className="text-gray-400">Subtotal</span><span className="text-gray-700">{fmt$$(pricing.subtotal)}</span></div>
                                <div className="flex justify-between"><span className="text-gray-400">Total Due</span><span className="font-semibold text-gray-900">{fmt$$(pricing.totalDue)}</span></div>
                                {production.estimatedCompletionDate && (
                                    <div className="flex justify-between"><span className="text-gray-400">Est. Completion</span><span className="text-gray-700">{production.estimatedCompletionDate}</span></div>
                                )}
                            </div>
                        </section>
                    </div>
                ...
                <section className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    >
                        <span className="text-[14px] font-semibold text-gray-700">Order Details — {lineItems.length} line item{lineItems.length !== 1 ? "s" : ""}</span>
                        <span className="text-gray-400 text-[18px]">{detailsOpen ? "−" : "+"}</span>
                    </button>
                    {detailsOpen && (
                        <div className="border-t border-gray-100">
                            <div className="grid grid-cols-[2fr_0.5fr_0.8fr_0.8fr] gap-4 px-6 py-2 bg-gray-50 text-[11px] text-gray-400 font-semibold uppercase tracking-widest">
                            ...
                            <div className="divide-y divide-gray-100">
                                {lineItems.map((li, i) => (
                                    <div key={i} className="grid grid-cols-[2fr_0.5fr_0.8fr_0.8fr] gap-4 px-6 py-3">
                                        <div>
                                            <p className="text-[13px] text-gray-800 font-medium">{li.description}</p>
                                            {li.comment && <p className="text-[11px] text-gray-400 mt-0.5">{li.comment}</p>}
                                        </div>
                                        <span className="text-[13px] text-gray-600">{li.quantity}</span>
                                        <span className="text-[13px] text-gray-600 tabular-nums">{fmt$$(li.unitPrice)}</span>
                                        <span className="text-[13px] font-semibold text-gray-900 tabular-nums">{fmt$$(li.totalPrice)}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
